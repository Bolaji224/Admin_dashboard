...
import api from "@/utils/axios"; // ✅ Use the shared axios instance
import {
  ChevronRight,
  Activity,
  Lock,
...
export default function AdminUserProfile() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await api.get("/admin/users"); // ✅ removed /v1
  ...
  const fetchUser = async (id: number) => {
    try {
      const res = await api.get(`/admin/users/${id}`); // ✅ removed /v1
  ...
  const updateProfile = async () => {
    ...
    try {
      await api.put(`/admin/users/${selectedUser.id}`, selectedUser); // ✅ removed /v1
  ...
  const approveUser = async () => {
    if (!selectedUser) return;
    try {
      await api.post(`/admin/users/${selectedUser.id}/approve`); // ✅ removed /v1
  ...
  const suspendUser = async () => {
    if (!selectedUser) return;
    try {
      await api.post(`/admin/users/${selectedUser.id}/suspend`); // ✅ removed /v1
      fetchUser(selectedUser.id);
    } catch (err) {
  ...
  const deleteUser = async () => {
    ...
    try {
      await api.delete(`/admin/users/${selectedUser.id}`); // ✅ removed /v1
  ...
  const changePassword = async (password: string) => {
    if (!selectedUser) return;
    try {
      await api.post(`/admin/users/${selectedUser.id}/change-password`, { // ✅ removed /v1
        password,
  ...
  const logoutAll = async () => {
    if (!selectedUser) return;
    try {
      await api.post(`/admin/users/${selectedUser.id}/logout-all`); // ✅ removed /v1
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Input
          ...
          onChange={(val) => setSelectedUser({ ...selectedUser, location: val })}
        />
        <Input
          label="Skills"
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 space-y-3">
          <h3 className="text-sm font-semibold text-red-700">Active Sessions</h3>
          ...
          <div className="relative w-full sm:w-80" ref={searchRef}>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
        ...
        {selectedUser && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <Sidebar
              ...
              user={selectedUser}
            />
            <div className="lg:col-span-3 bg-white rounded-xl border border-gray-200 p-6 md:p-8 shadow-sm">
              <ActiveComponent />
            </div>
          </div>
        )}
